...
app.post("/api/v1/chat/completions", async (req, res) => {
  ...
  try {
    ...
    const target = custom.baseUrl ? null : upstreamForModel(wanted);
    const ordered = custom.baseUrl ? keys : orderPoolLocal(keys, target, wanted);
    // Preserve OpenAI-shaped messages (multimodal parts, tool_calls, tool results)
    // instead of flattening them to { role, content:string }.
    ...
    // A 400/404 is only terminal when it came from the model's OWN upstream; a 404
    // from a fallback provider just means it does not host that model.
    const retryable = (st: number, servedNow: string) =>
      [401, 403, 429, 500, 502, 503, 504].includes(st) ||
      ([400, 404].includes(st) && (!!custom.baseUrl || !target || servedNow === target));
    ...
    for (let i = 0; i < ordered.length; i++) {
      ...
      if (wantStream) {
        ...
        const t0 = Date.now();
        const sresp = isAnth
          ? await openAnthropicStream({ apiKey: ordered[i].key, model: wanted, messages: openaiMessages, maxTokens: max_tokens, temperature })
          : await openOpenAIStream({ baseUrl: up.baseUrl, apiKey: ordered[i].key, model: wanted, messages: openaiMessages, maxTokens: max_tokens, temperature, tools, toolChoice });
        const sstatus = sresp ? sresp.status : 502;
        if (sresp && sresp.ok && sresp.body) {
          // Headers only after a 200 — rotation still works while streaming.
          res.writeHead(200, {
            "Content-Type": "text/event-stream; charset=utf-8",
            "Cache-Control": "no-cache, no-transform",
            Connection: "keep-alive",
            "X-Accel-Buffering": "no",
            "X-Edge-Provider": "Edge Router",
            "X-Edge-Upstream": served,
          ...
          keyCooldownUntilLocal.delete(kh); keyFail429Local.delete(kh); keyFailOtherLocal.delete(kh);
          res.end();
          return;
        }
        ...
        if (sstatus === 429) {
          ...
          if ((!custom.baseUrl && effHint !== "unknown" && effHint === served) || (!custom.baseUrl && !!itemBase)) {
            deadKeyIndexes.push(i); deadKeyPrefixes.push(ordered[i].key.slice(0, 8));
          } else {
            keyFailOtherLocal.set(kh, (keyFailOtherLocal.get(kh) || 0) + 1);
          }
        } else {
        ...
        if (!retryable(sstatus, served)) break;
        continue;
      }
      ...
      if (!retryable(r.status, served)) break;
    }
    ...
    for (let i = 0; i < ordered.length; i++) {
      ...
      try {
        ...
        const resp = await fetch(`${base.replace(/\/+$/, "")}/chat/completions`, {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${item.key}`, "HTTP-Referer": siteReferer(), "X-Title": "Edge Router" },
